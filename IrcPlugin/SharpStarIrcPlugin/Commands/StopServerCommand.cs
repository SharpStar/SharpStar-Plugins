...
{
    public class StopServerCommand : IrcCommand
    {
        ...
        public override void ParseCommand(IrcPlugin plugin, string channel, string nick, string[] args)
        {
            ...
            if (auth.Value != null)
            {
                ...
                if (perm != null || auth.Value.IsAdmin)
                {
                    ...
                    SharpStarMain.Instance.PluginManager.PassConsoleCommand("server", new[] { "stop" });

                    plugin.Irc.SendMessage(SendType.Message, channel, "Stopping Starbound server...");
